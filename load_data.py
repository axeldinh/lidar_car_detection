...
def load_data(processed=True):
    ...
    try:
        ...
        train_data = np.load("data/train.npz", allow_pickle=True)["train"]
        test_data = np.load("data/test.npz", allow_pickle=True)["test"]
        train_data, train_labels, test_data = preprocess_data(
            train_data[:, 0], train_data[:, 1], test_data
        )
...
class LidarModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if self.params["normalize"]:
            ...
            full_data = [
                (sample - coords_min) / (coords_max - coords_min)
                for sample in full_data
            ]
            test = [
                (sample - coords_min) / (coords_max - coords_min) for sample in test
            ]
...
def collate_fn(batch):
    ...
    padded_data = [
        np.pad(sample, ((0, padding), (0, 0)), "constant", constant_values=0)
        for sample, padding in zip(data, paddings)
    ]
